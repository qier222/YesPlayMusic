...
export default class {
  ...
  get currentTrackID() { return this._currentTrack?.id ?? 0; }
  ...
  _playAudioSource(source, autoplay = true) {
    ...
    this._howler.on('loaderror', (_, errCode) => {
      ...
      if (errCode === 3) {
        ...
        this._replaceCurrentTrackAudio(this.currentTrack, false, false)
            .then(replaced => {
              // 如果 replaced 为 false，代表当前的 track
              // 已经不是这里想要替换的track 此时则不修改当前的歌曲进度
              if (replaced) {
                this._howler?.seek(t);
                this.play();
              }
  ...
  async _getAudioSourceFromUnblockMusic(track) {
    ...
    /**
     * @type {import("@unblockneteasemusic/rust-napi").RetrievedSongInfo |
     *     null}
     */
    const retrieveSongInfo = await ipcRenderer.invoke(
        'unblock-music', store.state.settings.unmSource, track,
  ...
  _replaceCurrentTrack(
    ...
    return getTrackDetail(id).then(data => {
      ...
      return this._replaceCurrentTrackAudio(track, autoplay, true,
                                            ifUnplayableThen);
    });
  }
  /**
  ...
  _replaceCurrentTrackAudio(
      track, autoplay, isCacheNextTrack,
      ifUnplayableThen = UNPLAYABLE_CONDITION.PLAY_NEXT_TRACK) {
  ...
  _initMediaSession() {
    if ('mediaSession' in navigator) {
      ...
      navigator.mediaSession.setActionHandler(
          'nexttrack', () => { this._playNextTrack(this.isPersonalFM); });
  ...
  playPrevTrack() {
    ...
    if (trackID === undefined)
      return false;
    this.current = index;
    this._replaceCurrentTrack(trackID, true,
