...
const log =
    text => { console.log(`${clc.blueBright('[background.js]')} ${text}`); };
    ...
    dialog
        ...
        .then(result => {
          if (result.checkboxChecked && result.response !== 2) {
            win.webContents.send('rememberCloseAppOption',
                                 result.response === 0 ? 'minimizeToTray'
                                                       : 'exit');
          }
...
class Background {
  ...
  createExpressApp() {
    ...
    expressApp.use('/player', (req, res) => {
      this.window.webContents.executeJavaScript('window.yesplaymusic.player')
          .then(result => {
            res.send({
              currentTrack : result._isPersonalFM ? result._personalFMTrack
  ...
  createWindow() {
    ...
    const options = {
      ...
      backgroundColor : ((appearance === undefined || appearance === 'auto') &&
    ...
    if (this.store.get('window.x') && this.store.get('window.y')) {
      ...
      if (displays.length === 1) {
        ...
        if (x < bounds.x || x > bounds.x + bounds.width - 50 || y < bounds.y ||
    ...
    if (process.env.WEBPACK_DEV_SERVER_URL) {
      ...
      this.window.loadURL(
          showLibraryDefault ? `${process.env.WEBPACK_DEV_SERVER_URL}/#/library`
                             : process.env.WEBPACK_DEV_SERVER_URL);
      if (!process.env.IS_TEST)
        this.window.webContents.openDevTools();
    } else {
      createProtocol('app');
      this.window.loadURL(showLibraryDefault
  ...
  checkForUpdates() {
    ...
    autoUpdater.on('update-available',
                   info => { showNewVersionMessage(info); });
  }
  ...
  handleWindowEvents() {
    ...
    this.window.on(
        'moved', () => { this.store.set('window', this.window.getBounds()); });
  ...
  handleAppEvents() {
    app.on('ready', async () => {
      ...
      if (proxyRules) {
        this.window.webContents.session.setProxy(
            {proxyRules}, result => { log('finished setProxy', result); });
      }
      ...
      if (createDockMenu && app.dock)
        app.dock.setMenu(createdDockMenu);

      // create touch bar
      const createdTouchBar = createTouchBar(this.window);
      if (createdTouchBar)
        this.window.setTouchBar(createdTouchBar);

      if (isCreateTray)
        this.ypmTrayImpl = createTray(this.window);

      if (isCreateTaskbar)
        this.ypmTaskbarImpl = createTaskbar(this.window);
    ...
    app.on('quit', () => { this.expressApp.close(); });
